...
class Permission extends React.Component {
    ...
    handleDelete = (e: any, id: any) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/auth/permissions/${id}`)
                    .then(res => {
    ...
    loadData = () => {
        let url = `/auth/permissions?page=${this.state.data.currentPage}&limit=${this.state.limit}&sortBy=${this.state.sort_by}:${this.state.sort_dir}&searchBy=${this.state.search_by}&searchKey=${this.state.keyword}`
        axios.get(url)
            .then(res => {
                this.setState({
                    data: res.data
                            ...
                            <div className="card-body">
                                <div className="row">
                                    <div className="form-group col-lg-2">
                                        <Link href="/dashboard/permissions/create">
                                            <a className="btn btn-primary btn-block">Tambah</a>
                                        </Link>
                                    </div>
                                    <div className="form-group col-lg-2">
                                ...
                                <div>
                                    <table className="table table-striped">
                                        ...
                                        <tbody>
                                            {
                                                this.state.data.permissions && this.state.data.permissions.length > 0 ?
                                                    this.state.data.permissions.map((item: any) => {
                                                        return(
                                                            <tr key={item._id}>
                                                                <td>{ item.name }</td>
                                                                ...
                                                                <td>
                                                                    <Link href={`/dashboard/permissions/edit/${item._id}`}>
                                                                        <a title="Edit">
...

                                    {
                                        ...
                                        <div className="row mt-2">
                                            <div className="col-12 col-md-2 pt-2">
                                                Show { ((this.state.data.currentPage - 1) * this.state.limit) + this.state.data.permissions.length } Of { this.state.data.totalItems }
                                            </div>
