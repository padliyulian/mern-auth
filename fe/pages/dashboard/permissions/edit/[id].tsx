...
interface EditPermissionProps extends WithRouterProps {
}

class EditPermission extends React.Component<EditPermissionProps> {
    state = {
        name: '',
        guardName: '',
    ...
    handleLoad = () => {
        axios.get(`/auth/permissions/${this.props.router.query.id}`)
            .then(res => {
                if (res.data.status === 'success') {
                    this.setState({
                        name: res.data.permission.name,
                        guardName: res.data.permission.guardName
                    })
                }
            })
            .catch(error => {
                if (error.response.data.error) {
                    ...
                    this.props.router.push('/dashboard/permissions')
                }
            })
    }
    ...
    handleUpdate = (e: { preventDefault: () => void }) => {
        ...
        axios.patch(`/auth/permissions/${this.props.router.query.id}`, this.state)
            .then(res => {
                if (res.data.status === 'success') {
                    this.handleClear()
                    this.handleReset()
                            ...
                            <div className="col-lg-6">
                                <div className="card">
                                    ...
                                    <div className="card-body">            
                                        <form action="/auth/permissions" method="POST">
                                            <div className="row">
