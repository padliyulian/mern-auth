...
class Role extends React.Component {
    ...
    loadData = () => {
        let url = `/auth/roles?page=${this.state.data.currentPage}&limit=${this.state.limit}&sortBy=${this.state.sort_by}:${this.state.sort_dir}&searchBy=${this.state.search_by}&searchKey=${this.state.keyword}`
        axios.get(url)
            .then(res => {
                this.setState({
                    data: res.data
                                        ...
                                        <tbody>
                                            {
                                                this.state.data.roles && this.state.data.roles.length > 0 ?
                                                    (this.state.data.roles as any).map((item: any) => {
                                                        return(
                                                            <tr key={item._id}>
                                                                <td>{ item.name }</td>
                                                                ...
                                                                <td>
                                                                    <Link href={`/dashboard/roles/permissions/${item._id}`}>
                                                                        <a title="Permission">
                                                                    ...
                                                                    <Link href={`/dashboard/roles/edit/${item._id}`}>
                                                                        <a title="Edit">
...

                                    {
                                        ...
                                        <div className="row mt-2">
                                            <div className="col-12 col-md-2 pt-2">
                                                Show { ((this.state.data.currentPage - 1) * this.state.limit) + this.state.data.roles.length } Of { this.state.data.totalItems }
                                            </div>
